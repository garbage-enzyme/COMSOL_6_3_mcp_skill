...
- **Wave Optics 超表面陷阱** —— 正确创建 `ElectromagneticWavesFrequencyDomain`、`PeriodicStructure`、斜晶格 primitive cell、平移同构的 `FreeTri -> CopyFace -> FreeTet` 网格、Layered Drude 边界、波长-参数扫描（`plistarr`）及 PML/periodic-port 诊断。
