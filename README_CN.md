...
| --- | --- | --- |
| **opencode** | `skills/comsol-63-operations/SKILL.md` | 从 `~/.config/opencode/skills/` 自动加载，任务匹配 frontmatter `description` 时触发 |
| **Claude Code** | `CLAUDE.md`（项目根） | `CLAUDE.md` 内含 `@skills/comsol-63-operations/SKILL.md`，import 语法把完整 skill 注入上下文 |
| **Codex CLI**（及任何读 AGENTS.md 的工具：Gemini CLI、Cursor） | `AGENTS.md`（项目根） | `AGENTS.md` 被自动读取，其中指示 agent 在 COMSOL 任务时打开 `skills/comsol-63-operations/SKILL.md` |

唯一的内容源是 `skills/comsol-63-operations/SKILL.md`（markdown + YAML frontmatter）。`CLAUDE.md` 和 `AGENTS.md` 是薄指针 —— 无内容重复。
...
把仓库（或只 `CLAUDE.md` + `skills/`）放进你的项目。Claude Code 启动时从项目根读 `CLAUDE.md`，其中的 `@skills/comsol-63-operations/SKILL.md` import 会把 skill 内容注入上下文。
...
这些工具会自动读项目根的 `AGENTS.md`。把仓库 clone 进（或旁边）你正在做的项目，`AGENTS.md` 会在 COMSOL 任务出现时指示 agent 打开 `skills/comsol-63-operations/SKILL.md`。
