...
## 环境速查
- MCP 仓库（clientapi 校准 fork）：`github.com/garbage-enzyme/COMSOL_Multiphysics_MCP_6_3_Calibrated`。Python 环境 `D:\condaenvs\comsol-mcp`，启动 `python -m src.server`（cwd=项目目录）。
- **COMSOL 6.4**：`D:\COMSOL64\Multiphysics`，Java 21（6.4 自带 Temurin 21.0.7）。JPype 1.7.1 可用，**不需要** sitecustomize.py JVM patch（已改名 .bak）。
- CUDA Toolkit 12.4，RTX 3060 12GB。6.4 新增 **cuDSS**（CUDA Direct Sparse Solver），支持所有物理场直接求解器 GPU 加速。切 cuDSS：Stationary solver 子节点 `dDef.set('linsolver','cudss')`（默认 mumps）。消费级 GPU 加速有限（需百万 DOF + 数据中心 GPU 才显著 2-5x）。
