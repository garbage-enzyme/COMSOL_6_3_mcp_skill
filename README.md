# COMSOL MCP Skill — 6.4+ ClientAPI Operations

English | [中文](README_CN.md)

An agent skill that teaches AI coding assistants how to drive **COMSOL Multiphysics 6.4+** through the [comsol MCP server](https://github.com/garbage-enzyme/COMSOL_Multiphysics_MCP_6_4_Calibrated) with MPh 1.3.1 standalone / `clientapi`, and how to write or fix code in `src/tools/` when the API mismatches.
...
| --- | --- | --- |
| **opencode** | `skills/comsol-64-metasurface/SKILL.md` | Auto-loaded from `~/.config/opencode/skills/` when the task matches the frontmatter `description` |
| **Claude Code** | `CLAUDE.md` (project root) | `CLAUDE.md` contains `@skills/comsol-64-metasurface/SKILL.md`, an import that injects the full skill body into context |
| **Codex CLI** (and any AGENTS.md reader: Gemini CLI, Cursor) | `AGENTS.md` (project root) | `AGENTS.md` is read automatically; it instructs the agent to open `skills/comsol-64-metasurface/SKILL.md` for COMSOL tasks |

The single source of truth is `skills/comsol-64-metasurface/SKILL.md` (markdown + YAML frontmatter). `CLAUDE.md` and `AGENTS.md` are thin pointers — no content is duplicated.
...
- **Wave Optics metasurface pitfalls** — correct `ElectromagneticWavesFrequencyDomain` construction, `PeriodicStructure`, oblique primitive-cell topology, translation-congruent `FreeTri -> CopyFace -> FreeTet` meshes, layered Drude boundaries, wavelength-parametric sweeps (`plistarr`), and PML/periodic-port diagnostics.
...
Drop the repo (or just `CLAUDE.md` + `skills/`) into your project. Claude Code reads `CLAUDE.md` from the project root on startup, and the `@skills/comsol-64-metasurface/SKILL.md` import pulls the skill body into context.
...
These tools read `AGENTS.md` from the project root automatically. Clone the repo into (or beside) the project you're working in, and `AGENTS.md` will tell the agent to open `skills/comsol-64-metasurface/SKILL.md` whenever a COMSOL task comes up.
...
The skill is a single markdown file. Open `skills/comsol-64-metasurface/SKILL.md` and use it as a reference when writing COMSOL MCP code or driving the server by hand.

## Prerequisites

The skill assumes you already have:

- COMSOL Multiphysics 6.4 or newer installed and running
- The [comsol MCP server fork](https://github.com/garbage-enzyme/COMSOL_Multiphysics_MCP_6_4_Calibrated) configured in your AI tool (opencode / Claude Code / Codex)
...
The skill was distilled from real debugging sessions where AI assistants calibrated the MCP server's `src/tools/` for COMSOL standalone `clientapi`, then verified end-to-end through the MCP tool interface. The companion fork repo has the code changes and full `git log`.
